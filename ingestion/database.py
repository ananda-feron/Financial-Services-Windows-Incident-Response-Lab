...
CREATE TABLE IF NOT EXISTS events (
    ...
    technique_name TEXT,
    tactic TEXT,
    upstream_technique_id TEXT,
    upstream_technique_name TEXT,
    curated_technique_id TEXT,
    curated_technique_name TEXT,
    curated_by TEXT,
    curated_at TEXT,
...
PROVENANCE_COLUMNS = {
    "upstream_technique_id": "TEXT", "upstream_technique_name": "TEXT",
    "curated_technique_id": "TEXT", "curated_technique_name": "TEXT",
    "curated_by": "TEXT", "curated_at": "TEXT", "mapping_rationale": "TEXT",
}
...
def connect(path: Path) -> sqlite3.Connection:
    ...
    connection.execute("UPDATE events SET curated_technique_id = COALESCE(curated_technique_id, technique_id), curated_technique_name = COALESCE(curated_technique_name, technique_name)")
    connection.commit()
    return connection
...
def insert_events(connection: sqlite3.Connection, events: Iterable[dict[str, Any]]) -> tuple[int, int]:
    ...
    sql = """INSERT OR IGNORE INTO events (
        ...
        upstream_technique_id, upstream_technique_name, curated_technique_id,
        curated_technique_name, curated_by, curated_at, mapping_rationale,
    ...
    for event in events:
        ...
        primary_upstream = upstream[0] if upstream else {}
        connection.execute(sql, (
            key, event.get("timestamp"), event.get("computer"), event.get("username"),
            ...
            primary_upstream.get("id"), primary_upstream.get("name"),
            event.get("curated_technique_id"), event.get("curated_technique_name"),
            event.get("curated_by"), event.get("curated_at"), event.get("mapping_rationale"),
        ...
        if connection.total_changes > before:
            ...
            connection.execute("""UPDATE events SET
                upstream_technique_id = ?, upstream_technique_name = ?,
                curated_technique_id = ?, curated_technique_name = ?, curated_by = ?,
                curated_at = ?, mapping_rationale = ?, upstream_techniques_json = ?
                WHERE event_key = ?""", (
                primary_upstream.get("id"), primary_upstream.get("name"),
                event.get("curated_technique_id"), event.get("curated_technique_name"),
                event.get("curated_by"), event.get("curated_at"), event.get("mapping_rationale"),
                json.dumps(upstream, sort_keys=True), key,
            ))
    connection.commit()
